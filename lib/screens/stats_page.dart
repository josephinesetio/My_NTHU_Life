...
class _StatsPageState extends State<StatsPage> {
  ...
  final List<Map<String, dynamic>> _shopItems = [
    ...
    {"name": "NTHU Violet", "cost": 50, "previewColor": Colors.deepPurpleAccent},
  ];
  ...
  Future<void> _loadStatsAndShop() async {
    ...
    _equippedBorder = prefs.getString('equipped_border_${widget.studentID}') ?? "None";
    _unlockedBorders = prefs.getStringList('unlocked_borders_${widget.studentID}') ?? ["None"];

    // Reward Free Border implicitly if pet level milestones are met
    if (_pet != null && _pet!.currentLevel >= 5 && !_unlockedBorders.contains("Gold Scholar")) {
      _unlockedBorders.add("Gold Scholar");
      await prefs.setStringList('unlocked_borders_${widget.studentID}', _unlockedBorders);
    }
  ...
  Color _getBorderColor(String borderName) {
    switch (borderName) {
      case "Purple Haze": return Colors.purpleAccent;
      case "Gold Scholar": return Colors.amber;
      case "NTHU Violet": return const Color(0xFF7C3AED);
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text("Academic Identity", style: GoogleFonts.outfit(fontWeight: FontWeight.bold)),
        elevation: 0,
      ),
        ...
        child: Column(
          children: [
                ...
                children: [
                  Container(
                    ...
                    child: CircleAvatar(
                      ...
                      child: Icon(Icons.school_rounded, size: 40, color: cs.onPrimaryContainer),
                    ),
                  ),
                ...
                child: Column(
                  ...
                  children: [
                    Row(
                      ...
                      children: [
                        Text("Level ${_pet?.currentLevel ?? 1}", style: GoogleFonts.outfit(fontWeight: FontWeight.bold)),
                        Text("${_pet?.growthPoints ?? 0}/100 EXP", style: TextStyle(color: cs.onSurfaceVariant, fontSize: 13)),
                      ],
                    ),
            ...
            Align(
              ...
              child: Text("Avatar Border Customization", style: GoogleFonts.outfit(fontSize: 18, fontWeight: FontWeight.bold)),
            ),
                ...
                return Card(
                  ...
                  child: ListTile(
                    leading: CircleAvatar(
                      ...
                      child: isOwned ? const Icon(Icons.check, size: 16, color: Colors.white) : null,
                    ),
                    title: Text(name, style: GoogleFonts.outfit(fontWeight: FontWeight.w600)),
                    subtitle: Text(isOwned ? "Unlocked" : "Cost: $cost 🪙"),
                    trailing: ElevatedButton(
                      onPressed: isEquipped ? null : () => _handleShopAction(item),
                      style: ElevatedButton.styleFrom(
                        backgroundColor: isEquipped ? cs.surfaceContainer : (isOwned ? cs.secondaryContainer : cs.primary),
                        foregroundColor: isEquipped ? cs.onSurfaceVariant : (isOwned ? cs.onSecondaryContainer : cs.onPrimary),
                        elevation: 0,
                      ),
                      child: Text(isEquipped ? "Equipped" : (isOwned ? "Equip" : "Buy")),
                    ),
                  ),
                );
              },
            ...
            Align(
              alignment: Alignment.centerLeft,
              child: Text("Academic Settings", style: GoogleFonts.outfit(fontSize: 18, fontWeight: FontWeight.bold)),
            ),
            const SizedBox(height: 12),
              ...
              child: Column(
                children: [
                  ListTile(
                    ...
                    title: Text("Adjust Curriculum & Credits", style: GoogleFonts.outfit(fontSize: 15)),
                    subtitle: const Text("Modify structural semesters or initial course lists"),
                    trailing: const Icon(Icons.chevron_right_rounded),
                    onTap: () {
                      // Navigate back to credit layout interface (Index 1 on floating structure)
                      ScaffoldMessenger.of(context).showSnackBar(
                        const SnackBar(content: Text("Use the Credit navigation tab to update your transcripts!")),
                      );
                    },
                  ),
                ],
